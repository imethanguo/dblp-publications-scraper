...
- Merge mode: write into an existing `.js` file.
- Dedup against existing collection files.
- LLM-based `venueShort` generation.
- LLM-based tags generation with reference tags from cache.
...
- `max_workers`: parallel workers for metadata enrichment
- `per_item_sleep_seconds`: pause after each item log
- `dblp_request_interval_seconds`: DBLP request interval
- `enable_metadata_cache`: enable metadata cache
- `dedup_reference_dir`: dedup reference directory (default: `../collection`)
- `tags_reference_collection_dir`, `tags_reference_js_path`, `tags_reference_max_count`
- `tags_reference_cache_path`, `tags_reference_cache_force_refresh`
...
Tags generation rule (LLM):

- Tags are generated by LLM when publication metadata is processed.
- Preferred vocabulary is loaded from `.tags_reference_cache.json`.
- Tags cache is read-only during normal runs: scraper only reads from cache and does not update it.
...
Tags LLM can also read these environment variables:
...
- File: `.tags_reference_cache.json`
- Purpose: provide preferred tag vocabulary for LLM tag generation.
- Runtime behavior: read-only in scraper runs.
- If cache is empty or missing, scraper still runs but tag guidance may be weak.
